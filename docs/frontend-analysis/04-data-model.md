...
> ⚠️ JavaScript codebase — no declared types. Shapes reconstructed from API callers and store writes; ⚠️ fields should be confirmed against `app/core/schemas.py`.
...
Loaders deduped via `coalesce()` ([store.js](../../frontend/src/lib/store.js#L22-L30)); selectors at [store.js](../../frontend/src/lib/store.js#L196-L210).

## Context / client state

| Context | Shape | Storage key | Cross-tab sync | Consumers |
|---|---|---|---|---|
| `ThemeContext` | `{ theme:'light'\|'dark', setTheme, toggle }` | `quant_theme_v1` + `data-theme` | Yes | global chrome |
...
`MarketMeta` ([MarketContext.jsx](../../frontend/src/lib/MarketContext.jsx#L17-L26)): `{ code, label, short, flag, currency, symbol, suffix, defaultSymbol }`.

## API endpoint catalog (selected)

Full client: [lib/api.js](../../frontend/src/lib/api.js). Request/Response shapes ⚠️ inferred. Canonical catalog; referenced by `05` and `08`.
