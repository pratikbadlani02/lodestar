...
Target: `frontend/` — Lodestar trading dashboard SPA.
...
| TypeScript | N/A — no `tsconfig.json`, no static types | — |
| Module Federation | N/A — no federation plugin | — |
| Content Markdown/Mermaid | N/A — no `react-markdown`/`mermaid` dependency | — |
...
| Path aliases | None (relative imports) | [vite.config.js](../../frontend/vite.config.js) |
| Env var prefix | None used (`VITE_*` absent) | ⚠️ inferred from codebase |
| Vendor chunking | `vendor-react`, `vendor-charts`, `vendor-lwcharts`, `vendor-icons` | [vite.config.js](../../frontend/vite.config.js#L16-L22) |

## State & data layers

| Layer | Holds | Mechanism | Source |
|---|---|---|---|
| Zustand store | Live trading server state (control, health, account, positions, orders, alerts, strategies, backtests) | Coalesced loaders + WS invalidation | [lib/store.js](../../frontend/src/lib/store.js) |
| Context (client/UI) | theme, density, market (US/IN), active symbol + recents | React Context + localStorage | [lib/ThemeContext.jsx](../../frontend/src/lib/ThemeContext.jsx), [MarketContext.jsx](../../frontend/src/lib/MarketContext.jsx), [SymbolContext.jsx](../../frontend/src/lib/SymbolContext.jsx), [DensityContext.jsx](../../frontend/src/lib/DensityContext.jsx) |
| Page-local state | Market-data reads (snapshots, news, fundamentals, options, tape) | `useEffect` + `useState`, not cached | various `src/pages/*` |
| HTTP boundary | All `/api` calls | `fetch` wrapper in [lib/api.js](../../frontend/src/lib/api.js#L16-L46) | — |
...
In production the FastAPI process serves `frontend/dist` as static files with an SPA fallback (single origin); the static mount is added last so it cannot shadow `/api/*`. Source: [CLAUDE.md](../../CLAUDE.md).
