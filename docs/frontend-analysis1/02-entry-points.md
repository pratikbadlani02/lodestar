...
`RequireAuth` checks `sessionStorage.quant_token`; if absent, redirects to `/login?from=<encoded path>` ([App.jsx](../../frontend/src/App.jsx#L47-L57)). `Layout` is public; private pages are individually wrapped in `<Private>`.
...
| `/login?from=` | Post-login redirect target; honored only if same-origin relative path (`/`, not `//`) | [Login.jsx](../../frontend/src/pages/Login.jsx#L21-L24) |

## External entry points

N/A — no iframe embedding, MFE host-mount, or `remoteEntry.js`. SPA is launched only from its own `index.html`.
