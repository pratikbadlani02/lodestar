...
| `403`/`404` | ⚠️ No dedicated path — handled by generic `!ok` branch | — |
...
| Background load fail | store loader rejects | `try{}catch{}` + `finally` sets `*Loaded` | ⚠️ silently swallowed (no UI error) | [store.js](../../frontend/src/lib/store.js#L60-L96) |
...
| Missed WS event | tab hidden | 30s safety poll | eventual refresh | [store.js](../../frontend/src/lib/store.js#L173-L182) |
...
| Notifications | `sonner` toasts (e.g., backtest-completed) | [store.js](../../frontend/src/lib/store.js#L133-L139) |
| Telemetry | ⚠️ None — `componentDidCatch` has a "hook into Sentry later" placeholder only | [ErrorBoundary.jsx](../../frontend/src/components/ErrorBoundary.jsx#L13-L16) |
...
| Content parse errors | N/A — no Markdown/diagram pipeline | — |
