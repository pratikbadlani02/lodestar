...
APScheduler inside the FastAPI event loop.
...
alembic upgrade head                   # apply DB migrations (needs DATABASE_URL_SYNC reachable)
...
Operational CLI (`./manage.sh`) — wraps health checks, log tailing, migrations, and
the kill switch / liquidate emergency actions. Note its service-control commands assume
macOS `launchd` and are stale relative to the current deployment model (see below).
...
## Architecture caveat — read this first

The codebase migrated from **Celery (beat + worker)** to an **in-process APScheduler**,
but several support files still describe the old setup. Trust the code, not these docs:

- **Current truth**: `app/scheduler.py` + `app/tasks.py`. APScheduler runs inside the
  FastAPI event loop (`lifespan` in `app/main.py`), sharing the DB pool, broker HTTP
  client, and Redis connection with the request path. There is **no separate worker
  process**. `DEPLOY.md` and `requirements.txt` reflect this correctly.
- **Stale**: `README.md` (describes Celery + macOS launchd), `.env.example` and
  `app/core/config.py` (still define `CELERY_BROKER_URL` / `CELERY_RESULT_BACKEND` —
  unused for task execution), `manage.sh` (launchd `com.quant.*` services), and
  `launchd/`. Redis itself is still used, but only for control state (kill switch) and
  WebSocket pub/sub.
...
(replaces Celery `.delay()`) — `asyncio.create_task` with a strong-ref set so the GC
can't cancel them. CPU-heavy numpy/pandas simulation is pushed to a thread with
...
- **Kill switch** — Redis-backed (survives restarts), checked first in the risk gate.
  Toggle via `/api/control/*` or `./manage.sh kill`. `emergency_liquidate_all`
  (execution.py) cancels all orders and closes all positions.
...
Render blueprint in `render.yaml`; full walkthrough in `DEPLOY.md`. Redis is external
(Upstash) for control state + WebSocket pub/sub.
